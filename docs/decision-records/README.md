...
| ADR-XXXX (placeholder, #195) | RMM / managed-estate sources — Datto RMM, Datto BCDR, myITprocess (+ device-precedence revisit) | Proposed | 2026-06-15 | Adopt three managed-estate sources into bronze (`datto_rmm_devices`, `datto_bcdr_backups`, `myitprocess_recommendations`, front-end migration 0119); Datto RMM becomes a strong device authority **above m365/itglue but below `website`** (new order `website > datto_rmm > m365 > itglue`); SecretStore API keys, IT Glue path for the operational sources, fail-loud-on-missing-table. **ADR phase only** — collectors follow once frontend 0119 merges. |
