...
| **Azure ARM cloud-asset (CMDB, per-client)** | `Invoke-ImperionArmRequest` | `Get-ImperionCloudResource` (Subscription · ResourceGroup · Resource, fanned out per client tenant) | `cloud_subscriptions` / `cloud_resource_groups` / `cloud_resources` (pending FE migration) | daily | ADR-XXXX / #201, #216 |
