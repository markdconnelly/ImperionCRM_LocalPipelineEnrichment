...
| QBO vendor bill-payments | `scheduled-tasks/qbo/bill-payments.task.ps1` | daily | → `qbo_bill_payments` (authoritative payment fact, frontend ADR-0082; backend #105 reads it to set a timesheet Paid); DOUBLE-GATED — logs+exits until `qbo-access-token`/`qbo-realm-id` provisioned (QBO app reg, the standing time-tracking blocker) AND the front-end `qbo_bill_payments` migration lands; read-only, amount/vendor never logged (docs/integrations/quickbooks-online.md, ADR-0014) |
