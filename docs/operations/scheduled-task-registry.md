...
| Datto RMM devices | `scheduled-tasks/dattormm/devices.task.ps1` | daily | → `datto_rmm_devices` (patch/AV state, asset/software inventory; ADR-0018, issue #195). Front-end bronze migration 0119 SHIPPED + prod-applied (#674) — schema gate CLEAR. GATED on the API key — logs+exits until `datto-rmm-api-key`/Key Vault `Datto-RMM-API-Key` reachable (Mark). Auth = API-key→short-lived bearer exchange (connect helper owns it; token never logged). Downstream silver `device` merge precedence `website > datto_rmm > m365 > itglue` is front-end/cloud-Pipeline owned (docs/integrations/datto-rmm.md) |
| Datto BCDR backups | `scheduled-tasks/dattobcdr/backups.task.ps1` | daily | → `datto_bcdr_backups` (per-device backup posture, joins on device_uid; ADR-0018, #195). Migration 0119 SHIPPED + prod-applied. GATED on the API key — logs+exits until `datto-bcdr-api-key`/Key Vault `Datto-BCDR-API-Key` reachable (Mark). Auth = `Authorization: Bearer` header (URLs not secret-bearing) (docs/integrations/datto-bcdr.md) |
| myITprocess recommendations | `scheduled-tasks/myitprocess/recommendations.task.ps1` | daily | → `myitprocess_recommendations` (vCIO roadmap/QBR/assessment recommendations → account; ADR-0018, #195). Migration 0119 SHIPPED + prod-applied. Strategic/account data — flattens straight to Postgres, skips IT Glue (ADR-0006 §2). GATED on the API key — logs+exits until `myitprocess-api-key`/Key Vault `myITprocess-API-Key` reachable (Mark). Auth = `api_token` header (docs/integrations/myitprocess.md) |
