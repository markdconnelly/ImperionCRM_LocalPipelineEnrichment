...
# Hermetic test for Set-ImperionM365DeviceToBronze: verifies the ADR-0039 envelope remap into m365_devices.
...
Describe 'Set-ImperionM365DeviceToBronze' {
    It 'projects standard-envelope rows to external_ref/payload_bronze and upserts m365_devices with -NoChangeDetect' {
        InModuleScope ImperionPipeline {
            ...
            $rows = @(
                [pscustomobject]@{ external_id = 'd1'; raw_payload = '{"a":1}'; source = 'x'; tenant_id = 't1'; name = 'N'; content_hash = 'h' }
                [pscustomobject]@{ external_id = 'd12'; raw_payload = '{"a":2}'; source = 'x'; tenant_id = 't1'; name = 'M'; content_hash = 'i' }
            )
            $tally = $rows | Set-ImperionM365DeviceToBronze

            $script:captured.Table    | Should -Be 'm365_devices'
            ...
            $projected.external_ref   | Should -Be 'd1'
    ...
    It 'reuses a supplied connection and does not open or dispose one' {
        InModuleScope ImperionPipeline {
            ...
            $row = [pscustomobject]@{ external_id = 'd1'; raw_payload = '{}' }

            { $row | Set-ImperionM365DeviceToBronze -Connection $conn } | Should -Not -Throw
    ...
    It 'writes nothing and opens no connection for empty input' {
        InModuleScope ImperionPipeline {
            ...
            $tally = @() | Set-ImperionM365DeviceToBronze
    ...
    It 'honours -WhatIf: no upsert, no connection' {
        InModuleScope ImperionPipeline {
            ...
            $row = [pscustomobject]@{ external_id = 'd1'; raw_payload = '{}' }
            { $row | Set-ImperionM365DeviceToBronze -WhatIf } | Should -Not -Throw
            Should -Invoke Invoke-ImperionBronzeUpsert -Times 0
        }
    }
}
