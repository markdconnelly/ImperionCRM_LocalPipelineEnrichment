...
# Hermetic unit tests for Get-ImperionQboPurchase. Connect layer + context mocked in module scope.
...
Describe 'Get-ImperionQboPurchase' {
    BeforeEach {
    ...
    It 'flattens a Purchase to the qbo_purchases shape (amount is the payment fact)' {
        InModuleScope ImperionPipeline {
            Mock Invoke-ImperionQboRequest {
                , @([pscustomobject]@{
                        Id          = 'PUR-77'; TxnDate = '2026-06-05'; TotalAmt = 1480.50; PaymentType = 'Check'; DocNumber = 'CHK-1001'
                        AccountRef  = [pscustomobject]@{ value = 'A-1'; name = 'Operating Checking' }
                        EntityRef   = [pscustomobject]@{ value = 'V-9'; type = 'Vendor'; name = 'Contractor LLC' }
            ...
            $rows = @(Get-ImperionQboPurchase)
            $rows.Count | Should -Be 1
            $rows[0].txn_date | Should -Be '2026-06-05'
            $rows[0].total_amount | Should -Be '1480.5'
            $rows[0].payment_type | Should -Be 'Check'
            $rows[0].account_ref | Should -Be 'A-1'
            $rows[0].account_name | Should -Be 'Operating Checking'
            $rows[0].entity_id | Should -Be 'V-9'
            $rows[0].entity_type | Should -Be 'Vendor'
            $rows[0].entity_name | Should -Be 'Contractor LLC'
            ...
            $rows[0].external_id | Should -Be 'PUR-77'
    ...
    It 'survives unknown field names: flat columns land null, raw_payload keeps everything' {
        InModuleScope ImperionPipeline {
            Mock Invoke-ImperionQboRequest { , @([pscustomobject]@{ Id = 'PUR-9'; surpriseField = 'x' }) }
            $rows = @(Get-ImperionQboPurchase)
            $rows[0].total_amount | Should -BeNullOrEmpty
            $rows[0].entity_id | Should -BeNullOrEmpty
            $rows[0].external_id | Should -Be 'PUR-9'
            $rows[0].raw_payload | Should -Match 'surpriseField'
        }
    }

    It 'queries the Purchase entity, passes the incremental filter, and the realm to the connect layer' {
        InModuleScope ImperionPipeline {
            Mock Invoke-ImperionQboRequest { @() }
            Get-ImperionQboPurchase -ModifiedAfter '2026-06-01T00:00:00Z' | Out-Null
            Should -Invoke Invoke-ImperionQboRequest -Times 1 -ParameterFilter {
                $Query -match 'SELECT \* FROM Purchase' -and
                $Query -match "MetaData.LastUpdatedTime > '2026-06-01T00:00:00Z'" -and
                $RealmId -eq 'realm-999' -and $EntityProperty -eq 'Purchase'
            }
        }
    }
    ...
    It 'omits the WHERE clause for a full backfill (no ModifiedAfter)' {
        InModuleScope ImperionPipeline {
            Mock Invoke-ImperionQboRequest { @() }
            Get-ImperionQboPurchase | Out-Null
