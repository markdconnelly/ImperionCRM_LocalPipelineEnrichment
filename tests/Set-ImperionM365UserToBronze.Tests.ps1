...
# Hermetic test for Set-ImperionM365UserToBronze: verifies the ADR-0039 envelope remap into m365_contacts.
...
Describe 'Set-ImperionM365UserToBronze' {
    It 'projects standard-envelope rows to external_ref/payload_bronze and upserts m365_contacts with -NoChangeDetect' {
        InModuleScope ImperionPipeline {
            ...
            $rows = @(
                [pscustomobject]@{ external_id = 'u1'; raw_payload = '{"a":1}'; source = 'x'; tenant_id = 't1'; name = 'N'; content_hash = 'h' }
                [pscustomobject]@{ external_id = 'u12'; raw_payload = '{"a":2}'; source = 'x'; tenant_id = 't1'; name = 'M'; content_hash = 'i' }
            )
            $tally = $rows | Set-ImperionM365UserToBronze

            $script:captured.Table    | Should -Be 'm365_contacts'
            ...
            $projected.external_ref   | Should -Be 'u1'
    ...
    It 'reuses a supplied connection and does not open or dispose one' {
        InModuleScope ImperionPipeline {
            ...
            $row = [pscustomobject]@{ external_id = 'u1'; raw_payload = '{}' }

            { $row | Set-ImperionM365UserToBronze -Connection $conn } | Should -Not -Throw
    ...
    It 'writes nothing and opens no connection for empty input' {
        InModuleScope ImperionPipeline {
            ...
            $tally = @() | Set-ImperionM365UserToBronze
    ...
    It 'honours -WhatIf: no upsert, no connection' {
        InModuleScope ImperionPipeline {
            ...
            $row = [pscustomobject]@{ external_id = 'u1'; raw_payload = '{}' }
            { $row | Set-ImperionM365UserToBronze -WhatIf } | Should -Not -Throw
            Should -Invoke Invoke-ImperionBronzeUpsert -Times 0
        }
    }
}
