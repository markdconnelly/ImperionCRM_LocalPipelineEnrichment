...
# Hermetic test for Set-ImperionITGlueOrganizationToBronze: verifies the ADR-0039 envelope remap into itglue_companies.
...
Describe 'Set-ImperionITGlueOrganizationToBronze' {
    It 'projects standard-envelope rows to external_ref/payload_bronze and upserts itglue_companies with -NoChangeDetect' {
        InModuleScope ImperionPipeline {
            ...
            $rows = @(
                [pscustomobject]@{ external_id = 'o1'; raw_payload = '{"a":1}'; source = 'x'; tenant_id = 't1'; name = 'N'; content_hash = 'h' }
                [pscustomobject]@{ external_id = 'o12'; raw_payload = '{"a":2}'; source = 'x'; tenant_id = 't1'; name = 'M'; content_hash = 'i' }
            )
            $tally = $rows | Set-ImperionITGlueOrganizationToBronze

            $script:captured.Table    | Should -Be 'itglue_companies'
            ...
            $projected.external_ref   | Should -Be 'o1'
    ...
    It 'reuses a supplied connection and does not open or dispose one' {
        InModuleScope ImperionPipeline {
            ...
            $row = [pscustomobject]@{ external_id = 'o1'; raw_payload = '{}' }

            { $row | Set-ImperionITGlueOrganizationToBronze -Connection $conn } | Should -Not -Throw
    ...
    It 'writes nothing and opens no connection for empty input' {
        InModuleScope ImperionPipeline {
            ...
            $tally = @() | Set-ImperionITGlueOrganizationToBronze
    ...
    It 'honours -WhatIf: no upsert, no connection' {
        InModuleScope ImperionPipeline {
            ...
            $row = [pscustomobject]@{ external_id = 'o1'; raw_payload = '{}' }
            { $row | Set-ImperionITGlueOrganizationToBronze -WhatIf } | Should -Not -Throw
            Should -Invoke Invoke-ImperionBronzeUpsert -Times 0
        }
    }
}
