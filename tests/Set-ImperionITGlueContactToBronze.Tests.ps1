...
# Hermetic test for Set-ImperionITGlueContactToBronze: verifies the ADR-0039 envelope remap into itglue_contacts.
...
Describe 'Set-ImperionITGlueContactToBronze' {
    It 'projects standard-envelope rows to external_ref/payload_bronze and upserts itglue_contacts with -NoChangeDetect' {
        InModuleScope ImperionPipeline {
            ...
            $rows = @(
                [pscustomobject]@{ external_id = 'c1'; raw_payload = '{"a":1}'; source = 'x'; tenant_id = 't1'; name = 'N'; content_hash = 'h' }
                [pscustomobject]@{ external_id = 'c12'; raw_payload = '{"a":2}'; source = 'x'; tenant_id = 't1'; name = 'M'; content_hash = 'i' }
            )
            $tally = $rows | Set-ImperionITGlueContactToBronze

            $script:captured.Table    | Should -Be 'itglue_contacts'
            ...
            $projected.external_ref   | Should -Be 'c1'
    ...
    It 'reuses a supplied connection and does not open or dispose one' {
        InModuleScope ImperionPipeline {
            ...
            $row = [pscustomobject]@{ external_id = 'c1'; raw_payload = '{}' }

            { $row | Set-ImperionITGlueContactToBronze -Connection $conn } | Should -Not -Throw
    ...
    It 'writes nothing and opens no connection for empty input' {
        InModuleScope ImperionPipeline {
            ...
            $tally = @() | Set-ImperionITGlueContactToBronze
    ...
    It 'honours -WhatIf: no upsert, no connection' {
        InModuleScope ImperionPipeline {
            ...
            $row = [pscustomobject]@{ external_id = 'c1'; raw_payload = '{}' }
            { $row | Set-ImperionITGlueContactToBronze -WhatIf } | Should -Not -Throw
            Should -Invoke Invoke-ImperionBronzeUpsert -Times 0
        }
    }
}
