function Invoke-ImperionMetaMerge {
    ...
    .DESCRIPTION
        The bronze→silver merge for the Meta source (issue #126), owned by this repo on
        the posture-merge precedent (ADR-0010 / ADR-0013; grants from front-end
        migration 0075). Six idempotent, set-based steps — every INSERT is gated by
        NOT EXISTS or ON CONFLICT DO NOTHING, so a re-run converges and never
        duplicates (CLAUDE.md §6):
          ...
          5. Lead capture: ensure ONE lead_hook (kind facebook_dm, 'Facebook page
             inbox'); for each DISTINCT inbound DM sender, resolve the contact via
             contact_social_identity (platform facebook) or create a minimal contact +
             identity, then insert ONE lead_capture_event per sender (matched on
             hook + payload_bronze->>'from_id'). DM SENDERS ARE LEADS; commenters stay
             timeline-only (the 0075 contract).
