function Invoke-ImperionSocialEngagementMerge {
    <#
    .SYNOPSIS
        Merge Meta comment bronze into the silver social_engagement inbound store (slice H, #357).
    .DESCRIPTION
        ...
        (ADR-0026; the Meta 0075 / posture-merge pattern). v1 scope is COMMENTS on our posts
        (the existing facebook_comments + instagram_comments bronze, already collected by
        Invoke-ImperionMetaSocialSync). Brand MENTIONS are deferred — Meta has no
        brand-mention bronze table yet (front-end issue tracked in the PR); the kind enum
        ('comment'|'mention') and this merge are mention-ready for when that bronze lands.

        Two idempotent, set-based steps — each INSERT is gated by ON CONFLICT (channel,
    ...
    $started = Get-Date
    if (-not $PSCmdlet.ShouldProcess('meta comment bronze (facebook_comments, instagram_comments)', 'merge to social_engagement')) { return }
