function Set-ImperionAzureSubscriptionToBronze {
    <#
    .SYNOPSIS
        Write flattened Azure subscription rows into the azure_subscriptions bronze table.
    .DESCRIPTION
        Post-layer writer (CLAUDE.md §6). Takes the flat, fully-enveloped [PSCustomObject]
        rows produced by Get-ImperionAzureSubscription and upserts them via
        ...
        azure_subscriptions column set defined by front-end migration 0038 (display_name,
    ...
    .PARAMETER Row
        Flat bronze rows from Get-ImperionAzureSubscription (accepted from the pipeline).
    .PARAMETER Connection
        Optional open Npgsql connection to reuse. Opened from config + disposed when omitted.
    .PARAMETER Table
        Target bronze table. Defaults to azure_subscriptions (front-end migration 0038).
    .OUTPUTS
        The upsert tally { scanned; inserted; updated; unchanged }.
    .EXAMPLE
        Get-ImperionAzureSubscription | Set-ImperionAzureSubscriptionToBronze
    #>
    ...
    param(
        ...
        $Connection,
        [string] $Table = 'azure_subscriptions'
    )

    begin {
        # Exact column set of azure_subscriptions (front-end migration 0038): flat columns
