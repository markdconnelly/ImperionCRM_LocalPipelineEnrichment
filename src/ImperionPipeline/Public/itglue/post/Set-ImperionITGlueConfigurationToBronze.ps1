function Set-ImperionITGlueConfigurationToBronze {
    <#
    .SYNOPSIS
        Write flattened IT Glue configuration (device) rows into the itglue_devices bronze table (ADR-0039 shape).
    .DESCRIPTION
        Post-layer writer (CLAUDE.md §6) for a table that uses the per-source ADR-0039 shape
        instead of the standard envelope: itglue_devices (front-end migration 0036) keys on a
        ...
        merge into the unified device, not here. IT Glue configurations ARE the device/asset
        records, so each flat row from Get-ImperionITGlueConfiguration is projected down to
        { external_ref ← external_id, payload_bronze ← raw_payload } and upserted on
        external_ref with -NoChangeDetect (the table has no content_hash column; change is
    ...
    .PARAMETER Row
        Flat bronze rows from Get-ImperionITGlueConfiguration (accepted from the pipeline).
    .PARAMETER Connection
        Optional open Npgsql connection to reuse. Opened from config + disposed when omitted.
    .PARAMETER Table
        Target bronze table. Defaults to itglue_devices (front-end migration 0036).
    .OUTPUTS
