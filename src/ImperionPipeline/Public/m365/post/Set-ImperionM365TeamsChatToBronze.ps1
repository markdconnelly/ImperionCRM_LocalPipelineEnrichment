function Set-ImperionM365TeamsChatToBronze {
    <#
    .SYNOPSIS
        Write flattened cross-org Teams chat rows into the m365_teams_chats bronze table.
    .DESCRIPTION
        ...
        Get-ImperionM365TeamsChat (source 'm365_teams', cross-org filtered), renames the
        collector's `user` flat column to the table's `user_upn` (0065: `user` is a
    ...
    .PARAMETER Row
        Flat bronze rows from Get-ImperionM365TeamsChat (accepted from the pipeline).
    .PARAMETER Connection
        Optional open Npgsql connection to reuse. Opened from config + disposed when omitted.
    .PARAMETER Table
        Target bronze table. Defaults to m365_teams_chats (front-end migration 0065).
    .OUTPUTS
        The upsert tally { scanned; inserted; updated; unchanged }.
    .EXAMPLE
        Get-ImperionM365TeamsChat -User 'ada@imperionllc.com' -ClientDomain 'acme.com' | Set-ImperionM365TeamsChatToBronze
    #>
    ...
    param(
        ...
        $Connection,
        [string] $Table = 'm365_teams_chats'
    )

    begin {
        # Exact column set of m365_teams_chats (front-end migration 0065; collector `user`
        # lands in `user_upn`).
        $tableColumns = @(
