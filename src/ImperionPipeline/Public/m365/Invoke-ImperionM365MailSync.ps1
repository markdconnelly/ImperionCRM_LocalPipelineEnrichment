function Invoke-ImperionM365MailSync {
    <#
    .SYNOPSIS
        Hourly cross-org mail pull -> bronze (m365_mail_messages, migration 0065).
    .DESCRIPTION
        Promoted from scheduled-tasks/m365/mail.task.ps1 per ADR-0007 (cmdlet-first; no
        loose entry scripts). Composes one get + one post: Get-ImperionM365Mail piped to
        Set-ImperionM365MailToBronze. Single-tenant against the Imperion company tenant
        (Mark's 2026-06-11 authorization; GDAP fan-out deferred — assume clients grant data
        in the same formats). Read-only Graph. Requires Initialize-ImperionContext.
