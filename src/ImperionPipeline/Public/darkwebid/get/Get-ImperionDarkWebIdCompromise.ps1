function Get-ImperionDarkWebIdCompromise {
    ...
    .DESCRIPTION
        ...
        AUTH: Dark Web ID is a COMPANY credential in the system (Key Vault conn-company-darkwebid),
        not a local SecretStore secret — so the API key is passed in by the caller/task.
        CONFIRM BEFORE LIVE USE: base URL, path, and field names (email/breachSource/dateFound/
        exposedData/passwordType/…) are ASSUMPTIONS shared with the cloud Pipeline (ADR-0040).
    .PARAMETER ApiKey
        Dark Web ID API key (company credential), sent as the bearer.
    .PARAMETER Domain
        Optional client domain to scope the query.
    .PARAMETER TenantId
        Owning tenant stamped on each row; defaults to the partner tenant.
    .PARAMETER BaseUri
        Dark Web ID API base. Default 'https://api.darkwebid.com' (placeholder — confirm).
    .EXAMPLE
        Get-ImperionDarkWebIdCompromise -ApiKey $key -Domain 'acme.com'
    #>
    [CmdletBinding()]
    ...
    param(
        ...
        [string] $Domain,
        [string] $TenantId,
        [string] $BaseUri = 'https://api.darkwebid.com'
    )
    ...
    $records = Invoke-ImperionDarkWebIdRequest -ApiKey $ApiKey -Uri ('{0}{1}' -f $BaseUri.TrimEnd('/'), $path)
