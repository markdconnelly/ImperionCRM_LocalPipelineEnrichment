function Resolve-ImperionDattoBcdrApiKey {
    ...
    .DESCRIPTION
        Thin vendor adapter over Resolve-ImperionVendorSecret with the 'dattobcdr' catalog entry
        (issue #228, ADR-0018; the KQM/Voyage pattern, ADR-0009). Datto BCDR is an MSP-WIDE vendor
        credential — Imperion's own account, not a per-client key. The value is returned to the
        caller and never logged; Datto BCDR sends it as an `Authorization: Bearer` header (NOT the
    ...
    [OutputType([string])]
    param([string] $ApiKey)
    return Resolve-ImperionVendorSecret -Vendor 'dattobcdr' -Value $ApiKey
}
