function Get-ImperionKnowledgeContract {
    ...
    Invoke-ImperionKnowledgeCompose -EntityType 'contract' -Connection $Connection -TenantId $TenantId `
        -EmptyMessage 'knowledge contracts: no autotask_contracts bronze found.' `
        -Query @'
...
'@ -Compose {
        ...
        [pscustomobject]@{
            ...
            source     = 'autotask'
            metadata   = @{ account = $contract.account_name; status = $contract.status }
        }
    }
}
